...
repeat_rnr:
    ...
    while (interface_iter != NULL) {
        ...
        for (; interface_iter != NULL;
            interface_iter = hash_map_get_next(radio->interface_map, interface_iter)) {
            get_coutry_str_from_code(radio->oper_param.countryCode, country);
            global_op_class = (unsigned int) country_to_global_op_class(country, (unsigned char)radio->oper_param.operatingClass);
            if (add_eid_rnr_bss(&interface_iter->u.ap.hapd, &reporting_interface->u.ap.hapd,
                    &tbtt_count, &len, &eid, &tbtt_count_pos, tbtt_info_len,
                    global_op_class, mld_update, interface_iter == tx_interface))
                break;
        }
        pthread_mutex_unlock(&g_wifi_hal.hapd_lock);
